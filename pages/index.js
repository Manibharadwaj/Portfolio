...
export default function Home() {
  ...
  return <>
    ...
    <div className="portfolio-shell">
      <header className="site-header"><Link className="brand" href="/" onClick={(event) => scrollToSection(event, "top")} aria-label="Mani Bharadwaj home">MB<span>.</span></Link><nav aria-label="Primary navigation"><button type="button" onClick={(event) => scrollToSection(event, "work")}>Work</button><button type="button" onClick={(event) => scrollToSection(event, "experience")}>Experience</button><Link href="/blog">Writing</Link></nav><div className="header-actions"><a className="header-resume" href="/Resume.pdf" target="_blank" rel="noreferrer">Resume <FiArrowUpRight aria-hidden="true" /></a><ExternalLink className="header-contact" href={`mailto:${profile.email}`}>Let&apos;s talk <FiArrowUpRight aria-hidden="true" /></ExternalLink></div></header>
      <main id="top">
        <section className="hero section-wrap" aria-labelledby="hero-title">
          <div className="hero-copy"><p className="eyebrow"><span /> {hero.eyebrow}</p><h1 id="hero-title">{hero.title}</h1><p className="hero-lead">{hero.lead}</p><div className="hero-actions"><button type="button" className="button button-primary" onClick={(event) => scrollToSection(event, "work")}>{hero.primaryCta.label} <FiArrowDown aria-hidden="true" /></button><a className="button button-secondary" href={hero.secondaryCta.href}>{hero.secondaryCta.label} <FiArrowUpRight aria-hidden="true" /></a></div><div className="hero-meta"><span><FiMapPin aria-hidden="true" /> {profile.location}</span><span>React · Next.js · Node.js · Python</span></div></div>
        ...
        <section className="work-section section-wrap" id="work" aria-labelledby="work-title"><div className="section-heading"><p className="eyebrow">Selected work</p><h2 id="work-title">Projects with real systems behind them.</h2><p>Three things I&apos;ve built that show how I think about products, edge cases, and the people using them.</p></div><div className="case-study-list">{caseStudies.map((project, index) => <article className={`case-study case-study-${index + 1}`} key={project.slug}><ProjectMedia project={project} /><div className="case-copy"><p className="case-index">0{index + 1} <span>{project.eyebrow}</span></p><h3>{project.name}</h3><p className="case-description">{project.description}</p><dl className="case-detail"><div><dt>The problem</dt><dd>{project.problem}</dd></div><div><dt>What I built</dt><dd>{project.solution}</dd></div></dl><ul className="outcome-list">{project.outcomes.slice(0, 2).map((outcome) => <li key={outcome}>{outcome}</li>)}</ul>{project.feedback && <blockquote className="project-feedback">{project.feedback}</blockquote>}<div className="case-footer"><div className="tech-list">{project.tech.map((item) => <span key={item}>{item}</span>)}</div><ExternalLink href={project.github} className="source-link">{project.repoLabel} <FiArrowUpRight aria-hidden="true" /></ExternalLink></div></div></article>)}</div></section>
